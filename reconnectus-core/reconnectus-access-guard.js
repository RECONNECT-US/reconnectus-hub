...
onAuthStateChanged(auth, user=>{
  ...
  clearTimeout(authSettlingTimer);
  ...
  // Give Firebase a brief moment to restore the signed-in session.
  // During this time, only the neutral loading screen is shown.
  showChecking("Restoring your secure session.");

  authSettlingTimer=setTimeout(()=>{
    if(accessDecisionMade) return;

    const restoredUser=auth.currentUser;
    if(restoredUser){
      verifyAccess(restoredUser);
      return;
    }
