...
import { getAuth, onAuthStateChanged, setPersistence, browserLocalPersistence } from "https://www.gstatic.com/firebasejs/12.0.0/firebase-auth.js";
import { getFirestore, doc, getDoc, setDoc, addDoc, collection, serverTimestamp } from "https://www.gstatic.com/firebasejs/12.0.0/firebase-firestore.js";
...
const trackOnly = String(scriptElement?.dataset.trackOnly || "").toLowerCase() === "true";

const app = getApps().length ? getApp() : initializeApp(firebaseConfig);
const auth = getAuth(app);
const db = getFirestore(app);

// Match RECONNECT-US Core: keep the Firebase sign-in in browser-local storage
// so protected apps on the same GitHub Pages site can restore the session.
try {
  await setPersistence(auth, browserLocalPersistence);
} catch (error) {
  console.warn("RECONNECT-US could not set local Auth persistence:", error);
}
...
let activeUser = null;
let activityVisitRef = null;
let activityStartedAt = 0;
let activityClosed = false;

function elapsedSeconds(){
  return activityStartedAt ? Math.max(0,Math.round((Date.now()-activityStartedAt)/1000)) : 0;
}

async function startActivityVisit(profile,user){
  if(activityVisitRef || !appId || !profile || !user || profile.role==="superadmin") return;
  const pendingId=localStorage.getItem("ru_pending_app_visit_id");
  const pendingStart=Number(localStorage.getItem("ru_pending_app_started_ms"))||0;
  activityStartedAt=pendingStart||Date.now();
  activityClosed=false;
  try{
    if(pendingId){
      activityVisitRef=doc(db,"activity",pendingId);
      localStorage.removeItem("ru_pending_app_visit_id");
      localStorage.removeItem("ru_pending_app_started_ms");
      await setDoc(activityVisitRef,{appId,page:location.pathname,pageTitle:document.title||"",lastSeen:serverTimestamp(),status:"open"},{merge:true});
      return;
    }
    activityVisitRef=await addDoc(collection(db,"activity"),{
      uid:user.uid,email:user.email||profile.email||"",name:profile.name||user.displayName||"",
      role:profile.role||"learner",area:profile.area||"",type:"app_visit",appId,
      sessionId:localStorage.getItem("ru_login_session_id")||"",
      page:location.pathname,pageTitle:document.title||"",startedAt:serverTimestamp(),lastSeen:serverTimestamp(),
      endedAt:null,durationSeconds:0,status:"open"
    });
  }catch(error){ console.warn("RECONNECT-US activity start failed:",error); }
}

async function updateActivityVisit(close=false){
  if(!activityVisitRef || activityClosed) return;
  if(close) activityClosed=true;
  try{
    await setDoc(activityVisitRef,{
      lastSeen:serverTimestamp(),durationSeconds:elapsedSeconds(),
      ...(close?{endedAt:serverTimestamp(),status:"closed"}:{})
    },{merge:true});
  }catch(error){ console.warn("RECONNECT-US activity update failed:",error); }
  if(close){ activityVisitRef=null; activityStartedAt=0; }
}

async function updateLoginSessionFromApp(){
  if(activeProfile?.role==="superadmin") return;
  const sessionId=localStorage.getItem("ru_login_session_id");
  const sessionUid=localStorage.getItem("ru_login_session_uid");
  const startedMs=Number(localStorage.getItem("ru_login_session_started_ms"))||0;
  if(!sessionId||sessionUid!==activeUser?.uid||!startedMs) return;
  try{
    localStorage.setItem("ru_login_session_last_ms",String(Date.now()));
    await setDoc(doc(db,"activity",sessionId),{
      lastSeen:serverTimestamp(),durationSeconds:Math.max(0,Math.round((Date.now()-startedMs)/1000)),status:"open"
    },{merge:true});
  }catch(error){console.warn("RECONNECT-US login-session update failed:",error)}
}

async function ensureLoginSession(profile,user){
  if(!profile||!user||profile.role==="superadmin") return;
  const existingId=localStorage.getItem("ru_login_session_id");
  const existingUid=localStorage.getItem("ru_login_session_uid");
  if(existingId&&existingUid===user.uid) return;
  try{
    const startedMs=Date.now();
    const sessionRef=await addDoc(collection(db,"activity"),{
      uid:user.uid,email:user.email||profile.email||"",name:profile.name||user.displayName||"",
      role:profile.role||"learner",area:profile.area||"",type:"sign_in",appId:"reconnectus_core",
      page:location.pathname,startedAt:serverTimestamp(),lastSeen:serverTimestamp(),endedAt:null,
      durationSeconds:0,status:"open"
    });
    localStorage.setItem("ru_login_session_id",sessionRef.id);
    localStorage.setItem("ru_login_session_uid",user.uid);
    localStorage.setItem("ru_login_session_started_ms",String(startedMs));
    localStorage.setItem("ru_login_session_last_ms",String(startedMs));
  }catch(error){console.warn("RECONNECT-US could not create login session:",error)}
}
...
function startPresence(profile,user){
  ...
  writePresence(true);
  if(profile.role!=="superadmin") ensureLoginSession(profile,user).then(()=>startActivityVisit(profile,user));

  // Keep the user visible as online while the protected app is open.
  presenceTimer = setInterval(()=>{
    if(document.visibilityState === "visible"){
      writePresence(true);
      updateActivityVisit(false);
      updateLoginSessionFromApp();
    }
  },30000);
}
...
async function verifyAccess(user){
  ...
  try{
    ...
    // Pages with their own specialist permission checks can use the shared
    // activity tracker without also being forced through the app catalogue.
    if(trackOnly){
      accessDecisionMade = true;
      return allow(profile,user);
    }
...
(async function startAccessCheck(){
  ...
  try{
    ...
    // authStateReady() can finish a fraction before currentUser is exposed on
    // slower browsers/devices. Give Firebase one short extra chance to publish
    // the restored local session before showing Access Denied.
    ...
    if(!user){
      user = await new Promise(resolve => {
        ...
        const unsubscribe = onAuthStateChanged(auth, restoredUser => {
          if(finished || !restoredUser) return;
          finished = true;
...
document.addEventListener("visibilitychange",()=>{
  if(document.visibilityState === "visible"){
    writePresence(true);
    ensureLoginSession(activeProfile,activeUser).then(()=>{
      startActivityVisit(activeProfile,activeUser);
      updateActivityVisit(false);
      updateLoginSessionFromApp();
    });
  }else{
